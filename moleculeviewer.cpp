...
MoleculeViewer::MoleculeViewer(const std::vector<std::string> &args)
    : Application ("MoleculeViewer", args), multithread_(true), use_push_constants_(false),
      sim_paused_(false), sim_fade_(false), sim_(1000), camera_(2.5f), frame_data_(),
...
void MoleculeViewer::init_workers()
{
...
void MoleculeViewer::attach_shell(Shell &sh)
{
    Application::attach_shell(sh);
...
void MoleculeViewer::detach_shell()
{
    if (multithread_) {
        for (auto &worker : workers_)
            worker->stop();
    ...
    Application::detach_shell();
}

void MoleculeViewer::create_render_pass()
{
    ...
    assert_success(vkCreateRenderPass(dev_, &render_pass_info, nullptr, &render_pass_));
}

void MoleculeViewer::create_shader_modules()
{
...
#include "Hologram.vert.h"
    ...
    assert_success(vkCreateShaderModule(dev_, &sh_info, nullptr, &vs_));

#include "Hologram.frag.h"
    sh_info.codeSize = sizeof(Hologram_frag);
    sh_info.pCode = Hologram_frag;
    assert_success(vkCreateShaderModule(dev_, &sh_info, nullptr, &fs_));
}

void MoleculeViewer::create_descriptor_set_layout()
{
    if (use_push_constants_)
        return;
    ...
    assert_success(vkCreateDescriptorSetLayout(dev_, &layout_info,
                nullptr, &desc_set_layout_));
}

void MoleculeViewer::create_pipeline_layout()
{
    ...
    assert_success(vkCreatePipelineLayout(dev_, &pipeline_layout_info,
                nullptr, &pipeline_layout_));
}

void MoleculeViewer::create_pipeline()
{
    ...
    assert_success(vkCreateGraphicsPipelines(dev_, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &pipeline_));
}

void MoleculeViewer::create_frame_data(int count)
{
...
void MoleculeViewer::destroy_frame_data()
{
    if (!use_push_constants_) {
...
void MoleculeViewer::create_fences()
{
    ...
    for (auto &data : frame_data_)
        assert_success(vkCreateFence(dev_, &fence_info, nullptr, &data.fence));
}

void MoleculeViewer::create_command_buffers()
{
    ...
    for (size_t i = 0; i < cmd_pools.size(); i++) {
        ...
        assert_success(vkCreateCommandPool(dev_, &cmd_pool_info,
                    nullptr, &cmd_pool));
        ...
        assert_success(vkAllocateCommandBuffers(dev_, &cmd_info, cmds.data()));
    }
...
void MoleculeViewer::create_buffers()
{
    ...
    for (auto &data : frame_data_)
        assert_success(vkCreateBuffer(dev_, &buf_info, nullptr, &data.buf));
}

void MoleculeViewer::create_buffer_memory()
{
...
void MoleculeViewer::create_descriptor_sets()
{
    ...
    assert_success(vkCreateDescriptorPool(dev_, &desc_pool_info,
                nullptr, &desc_pool_));
    ...
    assert_success(vkAllocateDescriptorSets(dev_, &set_info, desc_sets.data()));
...
void MoleculeViewer::attach_swapchain()
{
...
void MoleculeViewer::detach_swapchain()
{
...
void MoleculeViewer::prepare_viewport(const VkExtent2D &extent)
{
    extent_ = extent;
...
void MoleculeViewer::prepare_framebuffers(VkSwapchainKHR swapchain)
{
    // get swapchain images
    get(dev_, swapchain, images_);
    ...
    for (auto img : images_) {
        ...
        VkImageView view;
        assert_success(vkCreateImageView(dev_, &view_info, nullptr, &view));
        ...
        VkFramebuffer fb;
        assert_success(vkCreateFramebuffer(dev_, &fb_info, nullptr, &fb));
        framebuffers_.push_back(fb);
    }
}

void MoleculeViewer::update_camera()
{
...
void MoleculeViewer::draw_object(const Simulation::Object &obj, FrameData &data, VkCommandBuffer cmd) const
{
    if (use_push_constants_) {
...
void MoleculeViewer::update_simulation(const Worker &worker)
{
    sim_.update(worker.tick_interval_, worker.object_begin_, worker.object_end_);
}

void MoleculeViewer::draw_objects(Worker &worker)
{
...
void MoleculeViewer::on_key(Key key)
{
    switch (key) {
    case KEY_SHUTDOWN:
    case KEY_ESC:
...
void MoleculeViewer::on_frame(float frame_pred)
{
    auto &data = frame_data_[frame_data_index_];

    // wait for the last submission since we reuse frame data
    assert_success(vkWaitForFences(dev_, 1, &data.fence, true, UINT64_MAX));
    assert_success(vkResetFences(dev_, 1, &data.fence));
...
MoleculeViewer::Worker::Worker(MoleculeViewer &molview, int index, int object_begin, int object_end)
    : molview_(molview), index_(index),
      object_begin_(object_begin), object_end_(object_end),
      tick_interval_(1.0f / molview.settings_.ticks_per_second), state_(INIT)
{
}

void MoleculeViewer::Worker::start()
{
    state_ = IDLE;
    thread_ = std::thread(MoleculeViewer::Worker::thread_loop, this);
}

void MoleculeViewer::Worker::stop()
{
    {
...
void MoleculeViewer::Worker::update_simulation()
{
    {
        std::lock_guard<std::mutex> lock(mutex_);
        ...
        // step directly
        if (!started) {
            molview_.update_simulation(*this);
            state_ = INIT;
        }
    }
    state_cv_.notify_one();
...
void MoleculeViewer::Worker::draw_objects(VkFramebuffer fb)
{
...
void MoleculeViewer::Worker::wait_idle()
{
...
void MoleculeViewer::Worker::update_loop()
{
    while (true) {
        std::unique_lock<std::mutex> lock(mutex_);
